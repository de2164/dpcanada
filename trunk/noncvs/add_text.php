...
$projectid = 'projectID3e89f47fc6457';  // project that will continue through the rounds
$txt_files_dir = '/data/home/dpscans/susanskinner/koenig/';  // where the text files are. 1 file per page.
$username_for_page_events = '[offline]';     // use something containing [] if you want a fake user
$username_for_project_events = 'lpryor';    // who is running this script

$image_suffix = '.png'; // might be .jpg
$round = get_Round_for_round_id('F2');      // the round we want the loaded text to appear as the output of
$change_project_round = FALSE;              // set it to true if you want the project to move rounds
$next_round = get_Round_for_round_id('F1');  // the round we want project to end up in, unavailable. Only has any effect if $change_project_round is TRUE.
$prev_text_column_name = 'round4_text'; // $round->prevtext_column_name
$increment_page_count = FALSE;          // does the merger get their page count incremented?
$select_condition = "state != 'F2.page_saved'";    // how to select the pages to be dealt with. Use '1' to do all pages
