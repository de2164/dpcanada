...
while ( list($projectid, $state, $nameofwork, $deletion_reason) = mysql_fetch_row($res2) )
{
    ...
    $query = "
        SELECT
            ...
            MAX({$work_round->time_column_name})
        FROM $projectid
        WHERE {$work_round->user_column_name}='$username'";
    $res3 = mysql_query($query);
    if ( $res3 !== FALSE )
    {
        list( $n_saved, $n_latered, $n_with_diff, $time_of_latest_save ) = mysql_fetch_row($res3);
        // if the user hasn't saved any pages for this project
        // or the user worked on a page before our cutoff, skip it
        if( $n_saved == 0 || $time_of_latest_save < $time_limit )
        {
            continue;
        }
    ...
    // save the row, we won't output until after we collate and sort them
    $project_row[$time_of_latest_save]=
        "<tr>" . 
        "<td $n_latered_bg><a href='$url'>$nameofwork</a></td>" .
        "<td nowrap>" . get_medium_label_for_project_state( $state ) . "</td>" .
        "<td>" . strftime( "%Y-%m-%d %T", $time_of_latest_save ) . "</td>" .
        "<td align='center'>$n_saved</td>" .
        "<td align='center' $n_latered_bg>$n_latered</td>" .
        "<td align='center' $n_w_diff_bg>$n_with_diff ($n_with_diff_percent%)</td>" .
        "<td $n_w_diff_bg>$diffLinkString</td>" .
        "</tr>";
...
echo "<p>" . sprintf(_("<b>%d</b> projects with pages saved in <b>%s</b> by <b>%s</b> within the last <b>%d</b> days."), $projects_with_pages_saved, $work_round->id, $username, $days) . "</p>";
