...
// or in this special case for the instrumented file, cpeel
// they can only access their own pages
if (!(user_is_a_sitemanager() || user_is_an_access_request_reviewer() || $pguser=="#####"))
{
    $username = $pguser;
}
...
// show form
echo "<form action='review_work_instrumented.php' method='GET'>";
echo "<table>";
...
$watch->start();
    ...
    $review_round_result = mysql_query("
       ...
       ");
$watch->stop();
$timings["${projectid}: count users in review round"]=$watch->read();
$watch->reset();
