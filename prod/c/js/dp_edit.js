...
function addEvent(obj, evType, fn) {
    ...
    alert("Note that one requested event listener couldn't be registered");
}
*/
...
//    addEvent(divproof,            "keydown",  eKeyDown);
//    addEvent(divproof,            "keyup",    eKeyUp);
//    addEvent(divproof,            "keypress", eKeyPress);
...
//    addEvent(divproof,            "mousemove", eMouseMove);
    ...
    // addEvent($('btntxtleft'),   "click",     eTextLeft);
    // addEvent($('btntxtright'),  "click",     eTextRight);
...
//                = ? window.innerHeight : document.documentElement.offsetHeight;
    ...
    //divleft.style.visible       = "visible";
    //divright.style.visible      = "visible";
    //divfratext.style.visible    = "visible";
    //divimage.style.visible      = "visible";
    //divcontrolbar.style.visible = "visible";
...
//    boxwidth  = divproof.offsetWidth;
    ...
    if(layout == 'horizontal') {
        ...
        // image is on top so height is barpos
...
function char_pickers(cgroup) {
    // var s = "<table class='tblpicker'>\n";
    // s += char_selector_row("A E I O U + Y CD LN RS TZ αβγ ἄἒ ἠό ίύ ώῤ Ћћ Ѡѡ");
...
function xy_in_divfratext(x, y) {
    return xy_in_div(divfratext, x, y);
}

function xy_in_divimage(x, y) {
    return xy_in_div(divimage, x, y);
}
*/
...
//    addEvent(formcontext.btnreplace, "click", eReplaceWordClick);  //this is a bogus function
    ...
    formcontext.btnbad.style.display = 
        ['good', 'suggested', 'adhoc', 'flagged', 'regex', 'suspect']
            .indexOf($("mode").value) >= 0
        ? "inline-block"
        : "none";
    formcontext.btnrecompute.style.display = 
        ...
        ? "inline-block"
        : "none";
//    formcontext.btnreplace.style.display = "inline-block";  //calling bogus eReplaceWordClick()

    // if($("mode").value == "regex") {
        // if($("txtfind") && $("txtfind").value != "") {
            // qry = {};
            // qry['querycode']    = 'regexcontext';
            // qry['projectid']    = $("projectid").value;
            // qry['word']         = $("txtfind").value;
            // qry['flags']        
                    // = ( $("chkignorecase").value == "on"
                        // ? "ui" 
                        // : "u");
            // writeAjax(qry);
        // }
...
function awcSuspectToBadWord(i) {
    var qry = {};
    qry['querycode']    = "suspecttobadword";
    qry['projectid']    = formcontext.projectid.value;
    qry['langcode']     = formcontext.langcode.value;
    qry['word']         = formcontext.tblcontext.options[i]
                            .value.substr(2);
...
function show_wordcheck(rsp) {
    var wclass, wccount = 0;
    var i, spans, span, accepts;
