...
function pf$(id) {
    return proofdoc.getElementById(id);
}

function ctl$(id) {
    return ctrldoc.getElementById(id);
    // if(frames.ctrlframe) {
        // return frames.ctrlframe.document.getElementById(id);
    // }
    // else {
        // return null;
    // }
...
function eBodyLoad() {
    proofdoc        = frames.proofframe.document;
    ctrldoc         = frames.ctlsframe.document;
    formedit        = pf$("formedit");
    divleft         = pf$("divleft");
    divright        = pf$("divright");
    divfratext      = pf$("divfratext");
    divimage        = pf$("divimage");
    divsplitter     = pf$("divsplitter");
    divcontrolbar   = pf$("divcontrolbar");
    divFandR        = pf$("divFandR");
    tatext          = pf$("tatext");
    imgpage         = pf$("imgpage");
    ...
    addEvent(proofdoc,      "keydown",   eKeyDown);
    addEvent(proofdoc,      "keyup",     eKeyUp);
    addEvent(proofdoc,      "keypress",  eKeyPress);
    ...
    addEvent(proofdoc,            "mousemove", eMouseMove);
    addEvent(pf$('linksync'),     "click",     eToggleSync);
    addEvent(pf$('linkzoomin'),   "click",     eZoomIn);
    addEvent(pf$('linkzoomout'),  "click",     eZoomOut);
    addEvent(pf$('linklayout'),   "click",     eSwitchLayout);
    addEvent(pf$('linkfind'),     "click",     eOpenFandR);
    addEvent(pf$('btnfind'),      "click",     eFind);
    addEvent(pf$('btnrepl'),      "click",     eReplace);
    addEvent(pf$('btnreplnext'),  "click",     eReplaceNext);
    addEvent(pf$('btnclose'),     "click",     eCloseFandR);
    addEvent(pf$('linkwc'),       "click",     eLinkWC);
    ...
    addEvent(pf$('badbutton'),    "click",     eToggleBad);
...
function Layout() {
    return GetLayout();
    // return pf$("formedit").layout.value;
}
...
//   }
    ...
    boxheight                       = proofdoc.body.offsetHeight
                                        - divstatusbar.offsetHeight;
    boxwidth                        = proofdoc.body.offsetWidth;
    ...
    if(Layout() == 'horizontal') {
        ...
        divFandR.style.top          =
        divcontrolbar.style.top     = px(barpos + divsplitter.offsetHeight);

        divright.style.top          = px(barpos + divsplitter.offsetHeight
                                                + divcontrolbar.offsetHeight);
        divright.style.left         = "0";
        divright.style.height       = px(boxheight - divleft.offsetHeight 
                                                   - divsplitter.offsetHeight
                                                   - divcontrolbar.offsetHeight);
        divright.style.right        = "0";
        // divright.style.right        = "100%";
    }
    else {
        ...
        divleft.style.top           = px(divcontrolbar.offsetHeight);
        divleft.style.left          = "0";
        // set width first so scrollbar doesn't disappear after setting height
        divleft.style.width         = px(barpos);
        divleft.style.height        = px(proofdoc.body.offsetHeight
                                        - divstatusbar.offsetHeight
                                        - divcontrolbar.offsetHeight);

        divsplitter.style.top       = px(divcontrolbar.offsetHeight);
        ...
        divright.style.top          = px(divcontrolbar.offsetHeight);
        divright.style.left         = px(divleft.offsetWidth + divsplitter.offsetWidth);
        divright.style.right        = "0";
        // divright.style.width        = px(boxwidth - barpos - divsplitter.offsetWidth);
...
function eCtlInit() {
    if(ctl$('charpicker')) {
        ctl$('selectors').innerHTML = char_selectors();
        ctl$('pickers').innerHTML   = char_pickers(' ');
    }
    invert_digraphs();
}
...
function char_selectors() {
    ...
    s += char_selector_row("A E I O UY CD LN R-Z αβγ ἄἒ ἠΐ όύ ώῤ ћ Ѫ + ❦");
    s += "\n</table>\n";
    return s;
}
...
function eCharClick(e) {
    ...
    if(t.className == "selector") {
        ...
        ctl$('pickers').innerHTML = char_pickers(c);
        return true;
    }
...
function ePickerOver(e) {
    ...
    if(! tgt.innerHTML) {
        ...
        ctl$('divchar').innerHTML = tgt.innerHTML;
        ctl$('divdigraph').innerHTML = ditgt;
    }
}
...
//    var kChar = e.charCode;
    ...
    if(typeof tatext.selectionStart == "number" 
        ...
        var selectionRange  = proofdoc.selection.createRange();
...
function SetSelection(start, end) {
    ...
    else if(proofdoc.selection && proofdoc.selection.createRange) {
        textInputRange = tatext.createTextRange();
...
function eSetImageUrl(url) {
    proofdoc.getElementById("scanimage")
        .src = url;
}
...
function eOpenFandR() {
    var _div = pf$('divFandR');
    if(_div.style.display == "none" || _div.style.display == "") {
        _div.style.display = "block";
        _div.style.top = px(divcontrolbar.offsetTop);
        if(SelectedText() != "") {
            pf$("txtfind").value = SelectedText();
            pf$("txtrepl").value = "";
        }
...
function set_regex() {
    var key = pf$('txtfind').value;
    if(pf$('chkm')) {
        key = key.replace(/[^\\]\./, "[\s\S]");
    }
    var flags = 'g' + (pf$('chki').checked ? 'i' : '');
...
function eReplace() {
    if(pf$('txtrepl').value.length == 0)
        return;
    var istart = SelectionBounds().start;
    var rstr = SelectedText().replace(regex, pf$('txtrepl').value);
    ReplaceText(rstr);
    SetCursor(istart + 1);
}
...
function adjust_wc_count(adj) {
    var n = parseInt(pf$("span_wccount").innerHTML, 10);
    pf$("span_wccount").innerHTML = (n + adj).toString();
}
...
function apply_scroll_pct() {
    ...
    var fromctl = pf$(_active_scroll_id);
...
function eSplitterDown(e) {
    ...
    // if(Layout() == "horizontal") {
        // divsplitter.style.top = px(divcontrolbar.clientTop + divcontrolbar.offsetTop);
        // divsplitter.style.left = "0";
        // divsplitter.style.width = "100%";
        // divsplitter.style.height = "4px";
    // }
    // else {
        ...
        // divsplitter.style.left = px(divcontrolbar.clientLeft + divcontrolbar.offsetLeft);
        // divsplitter.style.width = "4px";
    ...
    addEvent(proofdoc, "mouseup",   eSplitterUp);
}

function eSplitterUp(e) {
    var pct;
    _is_resizing = false;
    removeEvent(proofdoc, "mouseup", eSplitterUp);
    // adjust panels to match splitter
    if(Layout() == "horizontal") {
        divleft.style.height        = px(e.clientY - 1);
        divFandR.style.top          =
        divcontrolbar.style.top     = px(e.clientY);
        divright.style.top          = px(e.clientY + divcontrolbar.offsetHeight+4)
        pct = 100 * (e.clientY - divleft.clientTop) /
            (divleft.offsetHeight + divsplitter.offsetHeight + divright.offsetHeight);
    }
    else {
        divleft.style.width         = px(e.clientX-1);
        // divcontrolbar.style.left    = px(e.clientX);
        divright.style.left         = px(e.clientX);
        // divright.style.width....
        pct = 100 * (e.clientX - divleft.clientLeft) /
            (divleft.offsetWidth + divsplitter.offsetWidth + divright.offsetWidth);
    }
    SaveBarPct(pct);
    applylayout();
...
function GetFontFace() {
    ...
    return val == "" ? pf$('selfontface').value : val;
}

function GetFontSize() {
    var hv = Layout()[0];
    var val = getnamevalue(hv + "_fontsize");
    return val == "" ? pf$('selfontsize').value : val;
}

function SaveFontFace() {
    var hv = Layout()[0];
    setnamevalue(hv + "_fontface", pf$('selfontface').value);
}

function SaveFontSize() {
    var hv = Layout()[0];
    setnamevalue(hv + "_fontsize", pf$('selfontsize').value);
}
...
function setLayoutIcon() {
    pf$('switchlayout').src = ( Layout() == "horizontal"
            ? "gfx/horiz.png"
            : "gfx/vert.png" );
}
...
function show_wordcheck(rsp) {
    ...
    if(_accept_tags.length) {
        for(i = 0; i < _accept_tags.length; i++) {
            ...
            if(pf$(id)) {
                ...
                pf$(id).className = "accepted";
                pf$(id).style.className = "accepted";
            }
        }
    }
    ...
    pf$("span_wccount").innerHTML = (_wccount + _bwcount).toString();
...
function eWCMonitor(msg) {
    ...
    case 'savetemp':
        ...
        if(_rsp.alert) {
            ...
            alert("Save temp with no prompt.");
        }
        break;
...
function SelectionBounds() {
    ...
    if(typeof tatext.selectionStart == "number" 
        ...
        range = proofdoc.selection.createRange();
