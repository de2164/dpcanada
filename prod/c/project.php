...
if ($detail_level == 1) {
    ...
    detail_level_switch($projectid, $detail_level);
...
if($project->UserMayManage()) {
    echo "<div class='rfloat margined'>"
...
function decide_status($project) {
    ...
    if(! $project->UserMayProof()) {
        $msg = _("Unavailable for you to proof.");
...
function project_info_table($project) {
    ...
    echo_row_1_4( _("Language"),        $project->Language() );
    ...
    echo_row_1_4( _("Image URL"), h($project->ImageLink()));
    ...
    echo_row_1_4( _("Round/phase changed"), std_datetime($project->PhaseDateInt()));
...
//    assert($round);
    ...
    if($wlist == 1) {
        $sql = "
        SELECT image, state, fileid AS pagename, $timecol prooftime
        FROM $projectid
        WHERE $usercol = '$username' 
            AND (state LIKE '%out' OR state LIKE '%temp')
...
function offer_images($projectid) {
    ...
    echo "<form name='frmimages' method='POST'>
        "._('<h4>Images</h4>')."
        <p>$link</p>
